...
### T_Type — 거래 유형 7종 (실데이터는 2종)

실제 데이터는 `입고 636건 / 불출 439건` 2종뿐이지만, 설계상 7종이다.
**나머지 5종의 실데이터가 나중에 들어와도 코드를 고치지 않고 바로 동작하도록**
유형 정의를 `db.py` 한 곳(`TX_TYPES`)에 모았다. 유형을 추가하려면 이 표에 한 줄만 넣으면 된다.

| 유형 | 재고부호 | 수요 | 분류 | 의미 |
|---|:---:|:---:|---|---|
| 입고 | 0 | | in | 협력사 → 자재창고. **`Lot_tb.P_Qty`에 이미 반영돼 있어 0**<br>(거래행을 또 더하면 이중 계산이 된다) |
| 불출 | −1 | ✅ | out | 자재창고 → 현장. **유일한 '실소비'** |
| 반납 | +1 | | in | 현장 → 자재창고 복귀 |
| 불량 | −1 | | bad | 불량 판정으로 재고에서 제거 |
| 폐기 | −1 | | bad | 폐기 처분 |
| 이동 | 0 | | move | 창고 간 이동 (총량 불변, 위치만 변경) |
| 교환 | 0 | | move | 동수량 교체 (총량 불변) |

#### 파생 상수 — SQL에 유형명을 직접 쓰지 않는다
```python
LOT_DELTA   # LOT 잔량 차감식.  불출·불량·폐기는 +, 반납은 −, 나머지 0
DEMAND_IN   # 수요 지표 조건.   T_Type IN ('불출')       ← 일평균사용량·파레토·예측의 분자
TX_IN_SQL   # 입고측 집계 조건.  T_Type IN ('입고','반납')
TX_OUT_SQL  # 출고측 집계 조건.  T_Type IN ('불출','불량','폐기')
TX_META     # 화면에 넘기는 유형 메타(type/sign/demand/kind) → 필터 목록 자동 생성
```
> **수요와 출고를 구분한 이유** — 불량·폐기도 재고는 줄지만 *현장이 쓴 양*은 아니다.
> 이걸 수요에 넣으면 일평균 사용량이 부풀어 안전재고가 과대 산정된다.
> 그래서 **잔량 계산(LOT_DELTA)** 과 **수요 계산(DEMAND_IN)** 을 별도 상수로 분리했다.
> 이동·교환은 창고 총량이 변하지 않으므로 입고·출고 양쪽 집계에서 모두 제외한다.

#### 화면 쪽도 유형명을 못박지 않는다
유형별 색·아이콘을 `.tt-입고` 같은 **유형명 클래스**로 쓰면 새 유형에 스타일이 안 붙는다.
서버가 내려주는 `kind`(in/out/bad/move)로 매핑한다 (`txKind()` · `txIcon()`).
- `embed/tx_history.html` — 유형 드롭다운을 `TX_META`로 자동 생성(데이터 0건 유형은 비활성 표시),
  LOT 타임라인도 `isReceipt()`(kind:in & sign:0)로 최초 입고만 제외해 나머지 전 유형을 그린다
- `embed/users.html` · `dashboard.html` — 배지 색을 `kind` 기준으로 판정

> ✅ **주입 검증** — DB 복사본의 한 LOT(입고 4,000)에 불량100·폐기50·반납30·이동200·교환80을 넣고 확인:
> 잔량 `4000−100−50+30 = 3,880` 정확 일치, 이동·교환 280개는 잔량 미반영.
> 22개 조회 함수 전부 오류 0건, 45개 라우트 전부 정상, 타임라인에 7종 모두 표시됨.
...
```sql
현재고 = Lot_tb.P_Qty − 그 LOT의 순차감량
순차감량 = db.LOT_DELTA
         = SUM(CASE WHEN T_Type IN ('불출','불량','폐기') THEN  T_Num
                    WHEN T_Type IN ('반납')               THEN -T_Num
                    ELSE 0 END)
```
유형명을 SQL에 직접 쓰지 않고 `db.LOT_DELTA` 상수를 f-string 으로 끼워 넣는다.
(현재 데이터엔 불출만 있어 결과는 기존과 동일 — 회귀 확인 완료)
...
### ✅ 전 화면 실제 DB 연동 완료 (23개 = embed 22 + 대시보드)
| 화면 | 파일 | 내용 |
|---|---|---|
| **메인 대시보드** | `dashboard.html` | 전 화면 요약 관제. KPI 5 + 안전재고 긴급 · 창고별 재고 · 공급 리스크 · 최근 입출고 · 최근 발주 · 생산 병목 · ABC 집중도.<br>**embed 가 아니라 `/` 라우트에서 직접 `db.dashboard()` 를 받는다** |
| **자재 목록** | `embed/products.html` | 200종. 대분류탭(V/S/E/U/T)·필터(재고없음/대체품/A등급/미달/해외)·검색·정렬·CSV.<br>클릭 시 **품번체계 분해**·기본정보·재고·보유LOT·**완제품 사용처(BOM)**·안전재고.<br>목록의 **파란 품번을 직접 클릭**하거나 상세패널의 **[전체 LOT 이력 조회]** 버튼 → 모달로 그 자재의 전 LOT 일생(발주→입고→불출→생산투입)과 FIFO 위반 표시.<br>품번 셀은 `pidClick()` 이 `event.stopPropagation()` 으로 행 클릭(`pick()`)과 분리돼 상세패널 갱신 + 모달 열기를 함께 처리한다 |
| **안전재고 관리** | `embed/safety_stock.html` | 200종. 필터·정렬·검색·CSV.<br>클릭 시 등급근거(5점수)·**공식대입**·재검토주기·**실적기준 재계산 괴리** |
| **입고 처리** | `embed/inbound.html` | 발주 선택 → 발주 vs 입고 대조 + **LOT 채번 규칙** + 처리 흐름도 |
| **불출 처리** | `embed/disburse.html` | 자재·수량 선택 → **FIFO로 어느 LOT에서 얼마씩 꺼낼지** 자동 계산. 불출 후 안전재고 미달 경고 |
| **불출 승인** | `embed/approval.html` | 불출 건의 **금액·등급 기준 결재선 판정**(1~3단계). 담당자 처리만 실데이터, 상위 결재는 데이터 없음을 명시 |
...
| **발주 시뮬레이터** | `embed/simulator.html` | 실제 품목 200종 중 선택 → 발주수량·발주시점·리드타임·일평균사용량 슬라이더 조절.<br>180일 재고 추이를 **발주 有/無 두 선으로 비교**, 결품 시점·안전재고 미달일수·비용 산출 |
| **안전재고 일괄 갱신** | `embed/wizard.html` | SAFE_STOCK_DESIGN 의 갱신 절차를 **4단계 마법사**로 재현.<br>대상선정(199종) → 파레토 산정(결측 70 채움) → 등급 재판정(21종 변동) → 안전재고 재계산(187종).<br>⚠️ 조회 전용 — DB 를 수정하지 않고 "적용하면 이렇게 된다"만 보여준다 |
| **월간 리포트** | `embed/report.html` | 월 선택형 종합 리포트 8개월. 입고·불출·발주·생산 4개 카드 + **전월 대비 증감률**.<br>월별 추이 차트(선택 월 강조), 불출 상위 품목 / 발주 상위 협력사 랭킹 |
| **사용자 관리** | `embed/users.html` | 사원 30명 + **실제 처리 실적**(LOT 등록·입출고·작업지시). 직급 분포 막대, 담당 창고 분포, 최근 처리 이력.<br>개인정보 마스킹 정책을 화면에 명시 |
| **납기 리스크 레이더** | `embed/risk_radar.html` | **긴급도 × 영향도 2차원 매트릭스**(버블=발주 필요액).<br>수요예측과 관점이 다름 — 예측은 "언제 발주", 레이더는 "결품되면 얼마나 아픈가" |
| **발주 캘린더** | `embed/calendar.html` | 월 그리드 7개월(2025-07~2026-01). 날짜별 발주·입고 건수/금액 + 활동량 히트바.<br>날짜 클릭 시 그날의 발주 목록(→입고일)과 입고 LOT 목록 |
| **수요 예측 · 발주 시점** | `embed/forecast.html` | 불출 이력 기반 일평균 사용량 → 소진 예상일 → **발주 마감일** 산출.<br>긴급도 5단계, 권장 발주량(MOQ·포장단위 반영), 품목별 신뢰도 표시 |
| **LOT 상세 · 재고 노후화** | `embed/lot.html` | LOT 636개. 체류일수 분포 막대(클릭 필터), FIFO 위치 칩.<br>**FIFO 위반 102건 · 장기체화 63건** 판정 + 소진 게이지 + 입고 경로 |
| **재고 현황 지도** | `embed/stock_map.html` | 창고 5구역 배치도. 구역별 재고자산 게이지 + 소분류 스택바 + 미달 경고.<br>재고자산/수량 기준 전환, 구역 클릭 시 보관 품목 필터링 |
| **구매 발주 · 입고 대조** | `embed/purchase.html` | 발주 254건. 월별 추이 차트, 협력사·기간 필터.<br>클릭 시 **발주 수량 vs 실제 입고 수량 대조** + 발주→입고 흐름 + 생성 LOT |
| **생산 실적 · BOM 검증** | `embed/production.html` | 작업지시 488건. 월별 추이 차트.<br>클릭 시 **투입 자재를 BOM 표준과 대조** — 일치/대체/차이 판정. 일치율 95.9% |
| **협력사 · 납기 안정성** | `embed/suppliers.html` | 20개사. 리드타임 실측 통계(평균·중앙·편차·**변동계수**), 스파크라인.<br>**공급 리스크 5요소 점수화** + 공급품목 미달 현황 + 발주 실적 |
| **입출고 이력 · LOT 추적** | `embed/tx_history.html` | 거래 1,075건. 월별 추이 SVG 차트, **유형·창고 드롭다운 + 날짜 범위 지정 조회**(from~to, 최근 30일/90일/전체 퀵버튼).<br>유형 목록은 `TX_META` 로 자동 생성돼 새 유형이 들어오면 그대로 잡힌다.<br>클릭 시 **추적 타임라인**: 발주 → 입고(리드타임) → 이후 전 거래(불출·반납·불량·폐기·이동·교환) → 생산투입 → 잔량 |
...
```
자재 관리 : lot
입출고    : inbound, disburse, approval, scanner (입력 화면)
```

---

### 납기 리스크 = 긴급도 × 영향도 (`embed/risk_radar.html`)
수요 예측과 화면이 겹치지 않도록 **관점을 분리**했다.
```
수요 예측      "언제 발주해야 하는가"   → 시점 중심
리스크 레이더  "결품되면 얼마나 아픈가"  → 영향도 중심

긴급도 0~5  재고 0이면 5 · 발주마감 경과 4.5 · 14일내 3.5 · 45일내 2 · 90일내 1
영향도 0~5  자재등급(A2/B1/C0.5) + 투입완제품수(최대1.5)
            + 대체품없음(1) + 해외조달(0.5) + 장납기30일+(0.5)
리스크 = 긴급도×0.6 + 영향도×0.4
         3.6+ 위험 · 2.6+ 경고 · 1.6+ 주의 · 그 미만 안전
```
실측: 위험 67 · 경고 1 · 주의 35 · 안전 97
위험·경고 중 **대체품 없음 55종 · 해외조달 14종 · A등급 12종**. 해소 필요액 8.56억.

### 수요 예측 — 왜 단순 계산만 쓰는가 (`embed/forecast.html`)
자재당 불출 관측이 **1~6회(중앙값 2회)** 뿐이다. 회귀·계절성 분석을 하기엔 표본이 턱없이 부족하므로
**억지로 모델을 쓰지 않고** 방어 가능한 단순 계산만 한다. 대신 품목별 관측 횟수를 신뢰도로 노출한다.
```
일평균 사용량 d = 총 불출량 ÷ 관측기간(208일)
소진 예상일    = 현재고 ÷ d
발주 마감일    = 소진 예상일 − 리드타임      ← 0 이하면 이미 늦음
권장 발주량    = d × 리드타임 + 안전재고 − 현재고
                 → MOQ 및 포장단위로 올림
```
긴급도 — `재고소진`(재고 0) · `발주지연`(마감 경과) · `발주임박`(14일 내) · `주의`(45일 내) · `여유`
신뢰도 — 관측 4회+ `높음` · 2~3회 `보통` · 1회 `낮음` · 0회 `없음`

실측: 예측 가능 192종(이력 없음 8종 제외) · 재고소진 67 · 발주임박 1 · 주의 5 · 여유 119
권장 발주 **67종 · 8.56억원**. 신뢰도 높음 15 · 보통 137 · 낮음 40종.

> 월별 불출이 홀수달에 몰려 있으나(7월 82건 / 8월 41건 / 9월 115건 …) 이는 더미데이터 생성 패턴으로
> 보이므로 **계절성으로 해석하지 않는다.**

### FIFO 위반 판정 (`embed/lot.html`)
처음엔 **소진된 LOT 끼리의 출고 순서**를 비교했는데 위반이 0건으로 나왔다. 판정 기준이 틀렸다.
```
틀림: 나간 LOT 들이 입고순으로 나갔는가  → 항상 참이라 위반이 잡히지 않음 (0건)
맞음: 오래된 LOT이 아직 남아 있는데
      더 늦게 들어온 LOT이 먼저 나갔는가 → 위반 102건 검출
```
LOT 마다 `fifo_skipped`(건너뛰어진 횟수)를 기록한다.

실측 — 위반 **102건 / 174,689개 / 7.63억원**이 묶여 있음.
**장기체화(180일+) 63건 중 56건(89%)이 FIFO 위반 LOT** → 재고가 오래 묶이는 주원인이 선입선출 미준수.
예: `E02030002` LOT#1(2025-07-04)이 218일째 4,000개 그대로인데 #2~#6이 전부 먼저 출고됨.

체류일수 기준일은 `MAX(Transaction_tb.T_Date)` 를 쓴다. 실시간 `today` 를 쓰면
더미데이터라 체류일수가 비현실적으로 커진다.

### 창고 구역 집계 시 주의 (`embed/stock_map.html`)
`Lot_tb` 를 그대로 조인해 집계하면 **한 자재의 재고가 LOT 개수만큼 중복 합산**된다.
(L01 실제 47,075개 → 잘못 집계 시 161,450개)
```sql
-- 틀림: LOT 행마다 그 자재의 전체 재고가 더해진다
FROM Lot_tb l JOIN Product_tb p ON l.P_ID = p.P_ID

-- 맞음: 구역-자재 조합을 먼저 DISTINCT 로 뽑는다
WITH zp AS (SELECT DISTINCT Loc_ID, P_ID FROM Lot_tb) ...
```
검증: 5구역 전부 소분류 합계 = 구역 총계 일치. 복수 구역 보관 자재 0종.

실측 — 총 272,956개 / 17.98억 / 잔여 LOT 197 of 636.
창고는 대분류 전용: L01 밸브 · L02 센서 · L03 전장 · L04 탱크 · L05 튜브.
**미달 67종 = 재고 0종** (부분 부족이 아니라 전량 소진 상태).

### 발주 vs 입고 대조 (`embed/purchase.html`)
```
발주수량 = Purchase_Detail_tb.P_Qty
입고수량 = Lot_tb.P_Qty          (H_ID + P_ID 로 연결)
판정     = 일치 / 부족 / 초과 / 미입고
```
실측: 636개 품목라인 전부 입고 연결됨. **일치 624(98.1%) · 부족 12(1.9%) · 초과 0 · 미입고 0**
→ 부족 12건이 **예외 없이 포장단위(PkgUnit)의 정확한 배수**. 상자 단위 출하라 낱개 부족이 없다는 뜻.
   (9차 회의록의 "입고 일치율 95%, 5%는 PkgUnit 단위 부족 입고" 설계와 메커니즘 일치)

### 생산 대수 추정 + BOM 검증 (`embed/production.html`)
완제품 생산 수량이 원본에 없어 추정해야 한다. 두 번 고쳐 정확도를 올렸다.
```
1차) 작업지시 내 최소 투입량 = 1대분        → 일치율 87.1% · 불일치 423행
     최소 투입 자재가 1대당 1개가 아니면 전체 비율이 어긋남
2차) 표준BOM 자재들의 (실투입÷소요량) 중앙값 → 일치율 93.1% · 불일치 151행
     대체품으로 일부가 빠져도 중앙값이라 흔들리지 않음
3차) + 작업지시 내 자재별 합산                → 일치율 95.9% · 불일치  22행
     한 자재를 여러 LOT에서 꺼내면 행이 쪼개짐(FIFO 분할) → 합산 후 비교
```
판정 — `일치`(BOM 소요량과 동일) / `대체`(BOM 등록 대체품 투입) / `차이` / `미등록`
실측: 4,476개 자재 투입 중 일치 4,292 · 대체 162(139개 WO, 28.5%) · 차이 22(0.5%)

### 공급 리스크 점수 (`embed/suppliers.html`)
약속 납기일 컬럼이 데이터에 없으므로 **발주일→입고일 실측치의 통계**로 납기를 평가한다.
```
리드타임    = Purchase_Header_tb.P_Date → Lot_tb.Lot_Date  (건별 실측)
변동계수 CV = 표준편차 ÷ 평균 × 100
              리드타임 길이가 달라도 안정성을 공정하게 비교하기 위함
              (세종볼트 ±0.5일은 평균 2.6일 대비 19%, 글로벌디스플레이 ±5.7일은 61일 대비 9.3%)

공급 리스크 = 리드타임(최대6) + 납기변동(최대4) + A등급공급(최대4)
            + 안전재고미달(최대4) + 해외조달(2)
            12점 이상 '높음' · 7점 이상 '보통' · 그 미만 '낮음'
```
실측: 리스크 높음 2개사(글로벌디스플레이 15.4 · 비테스코센서 13.5) — 둘 다 해외·장納期·A등급 전량 공급.

### 대용량 화면 최적화 (`embed/tx_history.html`)
거래 1,075건에 자재명·규격·분류를 매 행 반복하면 응답이 1,489KB까지 커진다.
아래 두 가지로 **782KB(약 47% 감소)** 로 줄였다.
```
① 조회표 분리   자재정보는 tx_products()(200건)·tx_locations()로 따로 보내고
                화면에서 P_ID·Loc_ID 로 합친다 → 중복 제거
② used_in 절단  LOT별 생산투입 이력은 화면에 6건만 쓰므로
                상위 6건 + 총건수·완제품목록·총수량 요약만 전송
                (전량 4,535건 전송 시 +525KB)
```
같은 방식이 필요한 화면이 생기면 이 패턴을 따를 것.
...
### 대시보드 (`dashboard.html`)
embed 가 아닌 일반 템플릿이라 `EMBED_CONTEXT` 가 아니라 `/` 라우트에서 직접 데이터를 넘긴다.
`db.dashboard()` 는 **각 화면의 조회 함수를 그대로 재사용**해 집계하므로,
대시보드 숫자와 개별 화면 숫자가 어긋날 수 없다.
